...
export class AmpA4A extends AMP.BaseElement {
  ...
  onLayoutMeasure() {
    ...
    .then(creativeParts => {
      ...
      // Try devPublicKeyInfos in case we are in dev mode.
      // NOTE: Temporary until dev signing service is live.
      if (devPublicKeyInfos) {
        this.keyFetchPromises_.push(Promise.resolve(devPublicKeyInfos));
      }
      ...
      // Promise that will resolve to null if no successful validation happens.
      // Will call resolveValidation if a successful validation does happen.
      const invalidCreativePromise = Promise.all(
          this.keyFetchPromises_.map(
              keyFetchPromise => keyFetchPromise
              .then(keys =>
                keys && this.validateAdResponse_(
                    creativeParts.creative,
                    creativeParts.signature,
                    keys))
              .then(
                  ...
                  aggregate => {
                    aggregate && aggregate.errors &&
                        aggregate.errors.forEach(error => {
                          user().error('Amp Ad', error, this.element);
                        });
                  })
              )).then(() => null);
      // Race the two promises: Either validCreativePromise will resolve if a
      // successful validation occurs, or invalidCreativePromise will resolve to
      // null.
      return Promise.race([validCreativePromise, invalidCreativePromise]);
    })
  ...
  getPublicKeySet_() {
    ...
    return serviceNames.map(serviceName => {
      ...
      if (url) {
        return xhrFor(this.win)
            .fetchJson(url, {mode: 'cors', method: 'GET'})
            .then(keysContainer => {
              if (keysContainer && keysContainer.keys) {
                return keysContainer.keys.map(key => {
                  try {
